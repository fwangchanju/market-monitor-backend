...
패키지 목록을 외우는 것보다, 각 도메인이 무엇을 담당하는지를 아는 게 중요하다.
...
수집기 내부에서만 쓰는 중간 표현은 `dto/`가 아니라 private nested record로 둔다. `dto/`는 외부
경계(HTTP 요청·응답) 전용이다.
...
- 메서드명은 동사로 시작하고 역할이 드러나게: `message()` → `createMessage()`
