...
- **어느 거래소 가격인지 시간으로도 추론할 수 없다.** NXT는 정규장 시간에도 열려 있어서 10:00
  스냅샷의 SOR 값도 출처가 미상이다. "15:40 이후니까 NXT겠지"는 틀린 추론이다 — 실제로 한 번
  그렇게 틀렸다
...
**주권 기준 78.9%가 움직인다.** ETF가 희석하고 있었다. 나머지 21%는 0%로 표시되겠지만, 안 움직이는
쪽은 대체로 소형주라 **시총 가중평균에서는 영향이 더 작다.** 산술평균 토글에서만 눈에 띈다.

(이 숫자는 15:30 기준으로 잰 것이다. 기준선이 15:35로 확정됐으니 다시 재면 몇 종목 달라진다.)
...
**`stock_info`에 "NXT 거래 종목" 플래그를 다는 안은 보류한다.** 78.9%면 거래 없는 종목을 굳이
구분하지 않아도 그래프가 읽힌다. 실제 화면을 한 번 보고 나서 판단한다.
