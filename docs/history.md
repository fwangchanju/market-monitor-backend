...
2. 그 브랜치를 시험 배포하고 싶으면 → GitHub Actions "Run workflow"에서 **"Use workflow from"으로 그 브랜치를 고르고** `target=application`으로 수동 실행. 브랜치 이름 기준 태그(`:branch-<이름>`)를 쓰고, 그 태그 이미지가 이미 있으면(보통 1번에서 이미 빌드돼 있음) 재빌드 없이 바로 배포, 없으면 그때 빌드. 서버의 배포 스크립트 체크아웃도 이 선택한 브랜치 기준으로 맞춰짐 — 실제로 그 브랜치 코드가 프로덕션에서 돈다(트라이얼의 목적이 그것).
3. 문제 발견 시 → 같은 방식으로 "Use workflow from"을 `main`으로 바꿔서 다시 수동 실행 → 롤백. main 소스 기준으로 재빌드하므로 몇 분 정도 걸리지만, main 브랜치 자체는 트라이얼 동안 건드린 적이 없으므로 항상 안전한 상태로 돌아간다.
...
- 배포 스크립트(`deploy-application.sh`/`deploy-nginx.sh`/`deploy-renderer.sh`)는 전부 "어떤 이미지 태그를 pull할지"를 `IMAGE_TAG` 환경변수로 받는다. 기본값(`latest` 등)은 두지 않고 `: "${IMAGE_TAG:?...}"`로 미지정 시 즉시 실패 — 모든 호출 경로(Actions)가 항상 명시적으로 태그를 넘기게 설계했으므로, 값이 없다는 건 호출부 버그라 조용히 넘어가지 않고 바로 드러내는 게 맞다고 판단. 각 `docker-compose.yml`의 이미지 태그도 `${IMAGE_TAG}`로 변수화.
- 서버에서 매 배포마다 하는 `git fetch && git reset --hard origin/<브랜치>`는 **앱 코드를 가져오는 게 아니다** — 앱 코드는 Docker 이미지 안의 jar/정적파일이 100% 결정하고, 이 git 체크아웃은 오직 배포 스크립트/`docker-compose.yml` 파일 자체를 최신으로 유지하기 위한 것. `<브랜치>`는 `${{ github.ref_name }}`(= "Use workflow from"에서 고른 브랜치, push-to-main/repository_dispatch 트리거일 땐 자동으로 "main")이라, 일반 배포든 트라이얼이든 롤백이든 별도 입력 없이 항상 그 실행이 대상으로 하는 브랜치를 정확히 가리킨다.
- **수동 배포(`workflow_dispatch`, `target=application`/`nginx`/`renderer`/`all`)**: 예전엔 "전체 재빌드용 target"과 "재빌드 없이 기존 이미지만 pull하는 deploy_ref(트라이얼/롤백 전용)" 두 개의 입력이 따로 있었는데, `deploy_ref`는 타이핑으로 브랜치명을 직접 입력해야 해서 GitHub UI의 "Use workflow from" 드롭다운과 별개로 동작해 혼란스러웠다. 그래서 `deploy_ref`를 없애고, `target` 하나만 남긴 뒤 그 job들이 **"Use workflow from"으로 고른 브랜치를 기준으로** 이미지 태그를 계산(main→`main`/`latest`, 그 외→`branch-<이름>`)하고, GHCR에 그 태그가 이미 있으면 재빌드를 건너뛰도록 통합했다. 그 결과 "Use workflow from"만으로 트라이얼 배포/롤백이 되고(별도 텍스트 입력 불필요), `application`은 브랜치 push 시 자동으로 미리 빌드돼 있는 경우가 많아 대부분 재빌드 없이 빠르게 동작하며, main이 아닌 브랜치를 시험 배포해도 `:main`/`:latest` 태그(프로덕션이 실제로 참조하는 태그)는 건드리지 않는다.
...
마켓맵 상단에 "카테고리별 가중평균 등락률 랭킹" 바 차트(TradingView "1 DAY PERFORMANCE" 스타일)를 추가하려는 논의. 세 갈래로 나눠 정리했고, 그중 스냅샷/이력 부분(아래 2번)이 특히 까다로워서 설계만 잡고 구현은 보류함. 프론트 쪽 논의(1번 재사용 구조)는 market-monitor-frontend `docs/history.md`에 별도 기록.

**1) "지금" 기준 랭킹(구현 난이도 낮음, 보류 아님)**: 지정한 카테고리 레벨의 가중평균 등락률로 정렬해 바 차트로 표시. 프론트가 이미 `useMarketMap(market, isCustom)`으로 캐싱(`MARKET_DATA_CACHE`: staleTime/refetchInterval 60초)해둔 트리 데이터를 그대로 재사용하면 되고(같은 쿼리 키라 react-query가 자동으로 공유·중복요청 제거), 새 API 불필요. 다만 가중평균 계산 자체를 백엔드로 이관하기로 함(아래 참고) — 이관되면 이 화면도 그 필드를 그냥 읽기만 하면 됨.

**2) "직전 대비 급변" 랭킹(가장 복잡, 설계만 확정)**: 텔레그램 이미지 발송 주기(예: 08:10 vs 09:10)처럼 두 시점의 카테고리별 가중평균 등락률 %p 차이로 정렬. 이러려면 시점별 스냅샷 저장이 필요한데, 구간 중 종목이 A 카테고리→B 카테고리로 재배정되면 그 델타가 순수 가격변동이 아니라 구성 변경까지 섞인 값이 될 수 있다는 게 핵심 고민이었음. 결론:

- **델타 계산 자체는 문제없음** — 각 스냅샷은 "그 순간의 실제 카테고리 구성" 그대로 가중평균을 낸 것이므로 수치적으로 틀린 게 아니다. 문제는 해석(사용자가 "가격만 저만큼 움직였다"로 오해할 수 있음).
- **처음 검토했던 "구성원 지문(fingerprint) 스냅샷"안은 폐기** — 정렬된 종목코드 집합을 해시로 압축해서 두 시점 비교하면 "바뀜/안바뀜"은 알 수 있지만 "무엇이 바뀌었는지"는 알 수 없음.
- **최종안: `market_map_stock_category` 재배정 이력을 append-only 로그로 남긴다.** `MarketMapStockCategory.reassign()`(단건/일괄 배정 전부)이 호출될 때마다 `(stock_code, category_id_from, category_id_to, changed_at)`을 별도 이력 테이블에 기록. 삭제/수정 없는 순수 append.
  - "구성이 바뀌었나"는 **그 구간에 이벤트가 몇 번 찍혔는지가 아니라, 각 종목의 T1 시점 카테고리 vs T2 시점 카테고리를 직접 비교**해서 판단한다(각각 그 시점 이전 마지막 이벤트의 `category_id_to`로 조회). 이렇게 해야 "무엇이 바뀌었는지" 목록(편입/이탈)까지 같은 쿼리에서 그대로 나온다.
  - **어드민 종목관리 화면의 실행취소/다시실행(Ctrl+Z/Y)과 완전히 독립적** — undo/redo도 그냥 평범한 reassign 호출이라 이력에 똑같이 남지만, "시작-끝 상태만 비교"하는 판정 방식 덕분에 A→B(실수)→A(undo) 같은 왕복은 이벤트가 2번 찍혀도 T1=T2라 자동으로 "안 바뀜"으로 정리된다. 백엔드가 "이게 undo였는지" 알 필요도, undo/redo 쪽 코드를 손댈 필요도 전혀 없음.
  - 이 로그는 **기능이 실제로 나오기 전부터 미리 쌓아두기 시작**할 수 있음(reassign 시점마다 자동 기록이라 기능 출시와 무관) — 나중에 붙일 때 "로그 시작 이전 구간은 추적 불가"라는 손실이 없어짐.
- 가중평균 자체의 주기 스냅샷(예: 5분 수집 틱마다 카테고리별 값 저장)은 위 재배정 이력과 별개로 필요 — 이건 아직 스키마까지는 안 잡음.
