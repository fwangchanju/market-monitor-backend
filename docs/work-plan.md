...
- **이 지시서의 항목 번호를 커밋 단위로 착각하지 않는다.** 7-1~7-4는 설명의 단위이지 커밋의 단위가
  아니다. 단 **7-5는 별도 커밋이다** — 주제가 다르고 따로 되돌릴 이유가 있다
...
브랜치명 예: `claude/refactor/exception-log-and-retention`

백엔드 레포만 바꾼다.

> **7-5가 이 단계의 주제와 다르다.** 스냅샷 보존 기간을 30일에서 10일로 줄이는 한 줄짜리 변경인데,
> 예외 로그와 아무 관계가 없다. **배포를 한 번에 하려고 같이 태웠다.** 사용자가 정했다.
> 자세한 것은 7-5에 있다.
...
## 7-5. 스냅샷 보존 기간을 30일에서 10일로 줄인다

**예외 로그와 관계없는 변경이다.** 배포를 한 번에 하려고 이 단계에 같이 태웠다. **별도 커밋으로
분리한다** — 7-1~7-4와 이것은 따로 되돌릴 이유가 있다.
...
스냅샷 정리 배치는 `dry-run=true`로만 돌고 있고, 아직 한 번도 실삭제로 전환하지 못했다. 드라이런
로그가 삭제 술어를 증명하지 못했기 때문이다. 판정 기준은 "삭제 대상 건수 < cutoff 이전 전체 건수"인데
두 값이 같게 나왔다. cutoff(30일 전) 이전에 15:30 스냅샷이 한 건도 없어서 남길 것이 애초에 없었던
것이고, 술어가 뒤집혀 있어도 같은 숫자가 나온다.

5분 간격 수집을 시작한 것이 `ca2cde9`(2026-08-26)라 30일이 차기를 기다려야 했는데, 기다리는 대신
기간을 줄이기로 했다. 10일이면 cutoff 이전 구간에 15:30 스냅샷이 존재해서 바로 판정된다.

### 같이 고칠 것

`dry-run`은 프로퍼티지만 **`RETENTION_DAYS`는 자바 상수라 환경변수로 못 바꾼다.** 그래서 배포가
필요하고, "30일"을 문자로 적어둔 자리가 함께 어긋난다.

- `SnapshotRetentionScheduler`의 클래스 javadoc과 `calculateCutoff`의 주석. 둘 다 "30일"이 적혀 있다
- `SnapshotRetentionSchedulerTest` — 테스트 두 개가 **메서드 이름에 "30일"이 들어가고 기대 날짜를
  상수로 박아두었다.** 이름과 기대값을 함께 고친다. 빼먹으면 테스트가 깨지므로 조용히 누락되지는
  않는다

### 하지 않는 것

- **`dry-run`을 false로 바꾸지 않는다.** 그건 코드가 아니라 `~/env/market-monitor.env`의 환경변수이고,
  드라이런 로그를 사람이 한 번 읽고 판정한 뒤에 하는 일이다
- **드라이런 경로를 걷어내지 않는다.** 실삭제 전환이 끝난 뒤의 별건이다(`docs/backlog.md`)
- **초기 정리 SQL을 실행하지 않는다.** 쌓인 데이터를 한 번에 지우는 수동 작업이고 사용자가 직접 한다

### PR 설명에 적을 것

- 배치가 **KST 04:00**에 돈다는 것. 다음날 아침 드라이런 로그를 보려면 그 전날 안에 배포가 끝나야 한다
- 기간을 줄이면 첫 실삭제에서 지울 양은 오히려 **늘어난다**는 것. 초기 정리 SQL은 그대로 필요하다
