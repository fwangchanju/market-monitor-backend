...
>
> 사용자 판단 네 가지 중 세 개는 확정해서 본문에 반영했다. 백엔드 선배포, 6단계 자동 검증 추가,
> 15분 주기 유지다. **남은 것은 5-4와 `sectorAvailable` 관계 하나다.** 아래 「확정이 필요한 것」 참고.
>
> 순서: 남은 판단 반영 → PR #96 병합 → 구현 시작
...
**여기가 정해지기 전에는 구현을 시작하지 않는다.** 답이 채워지면 해당 절을 고치고 이 절은 지운다.

### 1. 5-4가 `sectorAvailable=false` 경로를 죽인다
