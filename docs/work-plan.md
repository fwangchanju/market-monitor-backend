...
>
> 남은 것
> - 5-4를 아직 안 썼다. 발송 직전에 `dataTime` 스냅샷이 있는지 확인하고 없으면 그 마켓 발송을
>   통째로 건너뛰는 조치다. 지금은 수집기가 예외를 던졌을 때만 발송이 막히고, 예외 없이 그 시각
>   스냅샷이 없으면 텍스트만 비고 이미지는 latest로 나간다. 08:20 이미지에 08:25 캡션이 붙는다
> - `grill-me` 검증(`docs/rules/process.md` 5번)을 아직 안 돌렸다
>
> 순서: 4단계 PR 리뷰·병합 → `grill-me` 검증 → 나온 것 정리(사용자 판단 필요한 건 확인받고)
> → 이 문서에 5-4와 검증 결과 반영 → PR #96 병합 → 구현 시작
