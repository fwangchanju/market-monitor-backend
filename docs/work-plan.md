...
>
> **7단계보다 먼저 하는 일이 하나 있다.** 섹터 "변화율" 그래프에 마켓 지수 바를 넣는 작업이다.
> 이 표의 단계가 아니라 별건이라 지시서를 `docs/backlog.md`의 「섹터 "변화율" 그래프에 마켓 지수
> 바를 넣는다」 항목에 뒀다. 백엔드와 프론트 두 레포를 바꾸고, 백엔드를 먼저 배포한다.
