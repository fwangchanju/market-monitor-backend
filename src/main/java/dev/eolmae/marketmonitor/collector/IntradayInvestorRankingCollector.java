...
public class IntradayInvestorRankingCollector {
			...
			for (IntradayInvestorType investorType : IntradayInvestorType.storableValues()) {
				for (IntradayRankingType rankingType : IntradayRankingType.values()) {
					try {
						...
						log.error("장중투자자랭킹 수집 실패: board={}, investor={}, ranking={}",
							board, investorType, rankingType, e);
					}
				}
			}
		}
	...
	private void collectForCombination(Board board, IntradayInvestorType investorType,
		...
		if (!existing.isEmpty()) {
			log.debug("장중투자자랭킹 이미 존재, 스킵: board={}, investor={}, ranking={}, snapshotTime={}",
				board, investorType, rankingType, snapshotTime);
			return;
		}
	...
	private enum MrktTp {
		KOSPI("001"), KOSDAQ("101");  // ka10065 mrkt_tp
		final String value;
