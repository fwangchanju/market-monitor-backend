...
public class LegacyCustomSectorController {
    ...
    public LegacySectorDeletePreview deletePreview(@PathVariable Long id) {
        SectorDeletePreview preview = customSectorService.deletePreview(id);
        return new LegacySectorDeletePreview(
                preview.sectorName(),
                preview.deletable(),
                preview.blockingStocks().stream()
                        .map(stock ->
                                new LegacyStockSectorItem(stock.stockCode(), stock.stockName(), stock.sectorName()))
                        .toList(),
                preview.deletableSectors());
    }
    ...
    public record LegacySectorDeletePreview(
            String categoryName,
            boolean deletable,
            List<LegacyStockSectorItem> blockingStocks,
            List<String> deletableCategories) {}

    public record LegacyStockSectorItem(String stockCode, String stockName, String categoryName) {}
}
