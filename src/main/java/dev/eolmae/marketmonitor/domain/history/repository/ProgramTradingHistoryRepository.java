...
public interface ProgramTradingHistoryRepository extends JpaRepository<ProgramTradingHistory, Long> {
	...
	@Query("SELECT h.snapshotTime FROM ProgramTradingHistory h WHERE h.stockCode = :stockCode AND cast(h.snapshotTime as LocalDate) = :date")
	List<LocalDateTime> findSnapshotTimesByStockCodeAndDate(@Param("stockCode") String stockCode, @Param("date") LocalDate date);
