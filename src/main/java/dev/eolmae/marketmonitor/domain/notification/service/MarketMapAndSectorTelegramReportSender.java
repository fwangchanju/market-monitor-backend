...
public class MarketMapAndSectorTelegramReportSender {
    ...
    public void send(LocalDateTime dataTime, Market market, boolean sectorAvailable) {
        ...
        if (sectorAvailable) {
            images.addAll(screenshotClient.capture(sectorPath(market), RenderTarget.CATEGORY_CHANGE_RATE.selector()));
        }

        String text = categoryRankingTextBuilder.buildRankingText(dataTime, MarketQuery.valueOf(market.name()));
        if (!sectorAvailable) {
    ...
    private String sectorPath(Market market) {
        return RenderTarget.CATEGORY_CHANGE_RATE.path() + "?market=" + market.name();
    }
}
