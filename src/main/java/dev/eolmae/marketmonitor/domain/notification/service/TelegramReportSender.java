...
// (CollectionScheduler) 책임 — 여기선 호출되면 무조건 캡처+발송한다. market은 캡처할 프론트 페이지가
// 어떤 마켓 상태로 렌더링돼야 하는지(URL 쿼리로 전달) + 캡션 텍스트 구성에 공통으로 쓰인다.
@Slf4j
...
public abstract class TelegramReportSender {
    ...
    protected abstract String buildText(LocalDateTime dataTime, Market market);

    public void send(LocalDateTime dataTime, Market market) {
        RenderTarget target = target();
        List<byte[]> images = screenshotClient.capture(target.path() + "?market=" + market, target.selector());
        byte[] combinedImage = ImageStitcher.stitchVertically(images);
        telegramClient.sendPhoto(telegramProperties.chatId(), combinedImage, buildText(dataTime, market));
        log.info("{}({}) 이미지 발송 완료: {}장 결합", target, market, images.size());
    }
}
