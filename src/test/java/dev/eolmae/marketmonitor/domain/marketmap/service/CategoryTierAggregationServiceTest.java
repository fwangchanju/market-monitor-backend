...
class CategoryTierAggregationServiceTest {
    ...
    private final CategoryTierAggregationService service =
    ...
    void aggregateByCategory_하위_카테고리_항목이_부모_합계에_재귀로_포함된다() {
        MarketValueTierThreshold large = tier(10L, "대형");
    ...
    void aggregateByCategory_시가총액_구간별로_따로_묶인다() {
        MarketValueTierThreshold large = tier(10L, "대형");
        MarketValueTierThreshold small = tier(20L, "소형");
    ...
    private MarketValueTierThreshold tier(Long id, String label) {
        MarketValueTierThreshold threshold = MarketValueTierThreshold.create(label, 0L, false);
